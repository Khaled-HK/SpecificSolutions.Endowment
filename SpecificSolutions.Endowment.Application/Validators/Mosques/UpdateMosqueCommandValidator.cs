...
{
    public class UpdateMosqueCommandValidator : BaseValidator<UpdateMosqueCommand>
    {
        public UpdateMosqueCommandValidator()
        {
            RuleFor(x => x.Id)
                .NotEmpty().WithMessage("معرف المسجد مطلوب");
            ...
            RuleFor(x => x.MosqueDefinition)
                .IsInEnum().WithMessage("تعريف المسجد غير صحيح");

            RuleFor(x => x.MosqueClassification)
                .IsInEnum().WithMessage("تصنيف المسجد غير صحيح");
