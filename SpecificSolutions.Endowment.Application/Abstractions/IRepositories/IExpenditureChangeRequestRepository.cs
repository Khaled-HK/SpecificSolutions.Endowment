...
namespace SpecificSolutions.Endowment.Application.Abstractions.IRepositories
{
    ...
    {
        Task<ExpenditureChangeRequest> GetByIdAsync(Guid id, CancellationToken cancellationToken);
        new Task<IEnumerable<ExpenditureChangeRequest>> GetAllAsync(CancellationToken cancellationToken);
        new Task AddAsync(ExpenditureChangeRequest request, CancellationToken cancellationToken);
        Task UpdateAsync(ExpenditureChangeRequest request);
