...
export default {
    ...
    data() {
        return {
            ...
            ScreenTitle: ' التخصصات الدراسية   ',
            ScreenTitleSingle: ' تخصص دراسي ',
            ScreenTitleSingleAl: 'التخصص الدراسي ',
    ...
    methods: {
        ...
        GetInfo(pageNo) {
            ...
            if (!this.Search)
                this.$blockUI.Start();
            this.$http.GetAcademicSpecializations(this.pageNo, this.pageSize, this.Search)
                .then(response => {
                    if (!this.Search)
                        this.$blockUI.Stop();
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.AddAcademicSpecializations(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
        ...
        submitEditForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.EditAcademicSpecializations(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
                            this.ClearForm(this.ruleForm);
        ...
        ChangeStatus(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.ChangeStatusAcademicSpecializations(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
        ...
        Delete(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.DeleteAcademicSpecializations(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
