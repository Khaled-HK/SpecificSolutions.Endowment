...
export default {
    ...
    data() {
        return {
            ...
            ScreenTitle: '  المدربين ',
            ScreenTitleSingle: '  مدرب ',
            ScreenTitleSingleAl: ' المدرب ',
            ...
            rules: {
                ...
                Percentage: this.$helper.Required(),
    ...
    methods: {
        ...
        GetInfo(pageNo) {
            ...
            if (!this.Search)
                this.$blockUI.Start();
            this.$http.GetInstructors(this.pageNo, this.pageSize, this.Search)
                .then(response => {
                    if (!this.Search)
                        this.$blockUI.Stop();
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.ruleForm.Percentage = Number(this.ruleForm.Percentage);
                    this.$blockUI.Start();
                    this.$http.AddInstructors(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
        ...
        OpentEditDialog(item) {
            ...
            this.ruleForm.Percentage = item.percentage;
            this.ruleForm.FacebookProfile = item.facebookProfile;
        },
        ...
        submitEditForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.ruleForm.Percentage = Number(this.ruleForm.Percentage);
                    this.$blockUI.Start();
                    this.$http.EditInstructors(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
                            this.ClearForm(this.ruleForm);
        ...
        ResetPassword(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.ResetPasswordInstructors(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
        ...
        SuspendssubmitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.SuspendsInstructors(this.SuspendsruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
