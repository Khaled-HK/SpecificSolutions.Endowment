...
export default {
    ...
    data() {
        return {
            ...
            ScreenTitle: '  الصحائف ',
            ScreenTitleSingle: '  صحيفة  ',
            ScreenTitleSingleAl: ' الصحيفة  ',
    ...
    methods: {
        ...
        GetInfo(pageNo) {
            ...
            if (!this.Search)
                this.$blockUI.Start();
            this.$http.GetClassesCertificates(this.pageNo, this.pageSize, this.Search, this.$parent.SelectedItem.id)
                .then(response => {
                    if (!this.Search)
                        this.$blockUI.Stop();
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.AddClassesCertificates(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
                            this.Back();
        ...
        submitEditForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.ruleForm.ClasseId = this.$parent.SelectedItem.id;
                    this.$blockUI.Start();
                    this.$http.EditClassesCertificates(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.Back();
                            this.ClearForm(this.ruleForm);
        ...
        ChangeStatus(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.ChangeStatusClassesCertificates(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
        ...
        Delete(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.DeleteClassesCertificates(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
