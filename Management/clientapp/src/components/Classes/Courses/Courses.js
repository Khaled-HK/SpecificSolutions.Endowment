...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                TeacherId: this.$helper.Required(),
                SubjectsId: this.$helper.Required(),
            },
        };
    },
    ...
    methods: {
        ...
        GetInfo(pageNo) {
            ...
            if (!this.Search)
                this.$blockUI.Start();
            this.$http.GetClassesCourses(this.pageNo, this.pageSize, this.Search, this.$parent.SelectedItem.id)
                .then(response => {
                    if (!this.Search)
                        this.$blockUI.Stop();
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.AddClassesCourses(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.GetInfo();
                            this.Back();
        ...
        OpentEditDialog(item) {
            ...
            this.ruleForm.TeacherId = item.teacherId;
            this.ruleForm.SubjectsId = item.subjectsId;
        ...
        submitEditForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.ruleForm.ClasseId = this.$parent.SelectedItem.id;
                    this.$blockUI.Start();
                    this.$http.EditClassesCourses(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.Back();
                            this.ClearForm(this.ruleForm);
        ...
        ChangeStatus(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.ChangeStatusClassesCourses(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
        ...
        Delete(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.DeleteClassesCourses(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
